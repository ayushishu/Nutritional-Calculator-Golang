...
var sugersLevels = []float64{45, 60, 36, 31, 27, 22.5, 18, 13.5, 9, 4.5}
...
var sugersLevelsBeverage = []float64{13.5, 12, 10.5, 9, 7.5, 6, 4.5, 3, 1.5, 0}
...
func (s SugerGram) GetPoints(st ScoreType) int {
	if st == Beverage {
		return getPointsFromRange(float64(s), sugersLevelsBeverage)
	}
	return getPointsFromRange(float64(s), sugersLevels)
}
...
func (sdu SodiumMilliGram) GetPoints(st ScoreType) int {
	return getPointsFromRange(float64(sdu), sodiumLevels)
}
...
func SaturatedFromSalt(saltMg float64) SodiumMilliGram {
...
func GetNutritionalscore(n NutritionalData, st ScoreType) NutritionalScore {
	...
	if st != Water {
		...
		negative = n.Energy.GetPoints(st) + n.Sugers.GetPoints(st) + n.SaturatedFattyAcids.GetPoints(st) + n.Sodium.GetPoints(st)
